...
const STRATEGY_LABELS: Record<PricingStrategyType, string> = {
  per_day: "Per Day",
...
function buildPayload(form: FormState): CreatePricingConfigPayload | UpdatePricingConfigPayload {
  const base = {
...
function getScopeBadgeStyle(scope: PricingScope): string {
  switch (scope) {
    case "organization":
...
function formatStrategyParams(config: PricingConfig): string {
...
export default function PricingConfigs() {
  ...
  const [formItems, setFormItems] = useState<Array<{ _id: string; name: string }>>([]);
  ...
  const [previewItems, setPreviewItems] = useState<Array<{ _id: string; name: string }>>([]);
  const [previewItemsLoading, setPreviewItemsLoading] = useState(false);
  const [previewItemSearch, setPreviewItemSearch] = useState("");

  // ── Fetch form items when create modal opens or scope changes ───────────
  useEffect(() => {
  ...
  const fetchConfigs = useCallback(async () => {
    ...
    try {
      ...
      // Handle response both as direct array or nested configs object
  ...
  const handleOpenEdit = async (config: PricingConfig) => {
    try {
      // Get the correct ID whether it's _id or id
          ...
          <div className="bg-zinc-900 border border-zinc-700 rounded-xl p-6 w-full max-w-lg shadow-2xl space-y-5 my-8">
                    ...
                    size={
                      Math.min(
                        formItems.filter(
                          (item) =>
                            !formItemSearch ||
                            item.name.toLowerCase().includes(formItemSearch.toLowerCase()),
                        ).length,
                        5,
                      ) || 1
                    }
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, strategyType: e.target.value as PricingStrategyType }))
                }
                className="w-full px-3 py-2 bg-[#1a1a1a] border border-[#333] rounded-lg text-white focus:outline-none focus:border-[#FFD700] transition-all"
              >
                {(Object.keys(STRATEGY_LABELS) as PricingStrategyType[]).map((s) => (
                  ...
                  size={
                    Math.min(
                      previewItems.filter(
                        (item) =>
                          !previewItemSearch ||
                          item.name.toLowerCase().includes(previewItemSearch.toLowerCase()),
                      ).length,
                      5,
                    ) || 1
                  }
