...
export function CreateMaterialPage({ onSuccess, onNavigateBack }: CreateMaterialPageProps) {
  ...
  const fingeredMaterials = materials.filter((material) => {
    ...
    const matchesCategory = (() => {
      ...
      const cat: any = (material as any).categoryId ?? (material as any).category ?? (material as any).modelId ?? (material as any).model;
      if (!cat) return false;
      let id: any = cat;
      if (Array.isArray(cat)) id = cat[0];
      if (typeof id === "object") id = id._id ?? id;
      if (typeof id === "string") return id === filters.categoryId;
      return String(id) === filters.categoryId;
    })();
            ...
            {showForm && (
              <MaterialForm
                ...
                initialData={
                  editingMaterial
                    ? (() => {
                        const catAny: any = (editingMaterial as any).categoryId ?? (editingMaterial as any).category ?? (editingMaterial as any).modelId ?? (editingMaterial as any).model;
                        let catId = "";
                        if (catAny) {
                          const first = Array.isArray(catAny) ? catAny[0] : catAny;
                          if (typeof first === "object") catId = first._id ?? String(first);
                          else catId = String(first);
                        }
                        return {
                          categoryId: catId,
            ...
            {loading ? (
              <div className="flex items-center justify-center h-64 bg-white rounded-lg">
                ...
                <MaterialList
                  ...
                  onEdit={(material) => { setEditingMaterial(material); setShowForm(true); }}
