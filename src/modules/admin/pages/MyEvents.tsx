...
export default function MyEvents() {
	...
	const fetchEvents = async () => {
		try {
			...
			if (response.status === 'success' && response.data) {
				const mappedEvents: Event[] = response.data.requests?.map((request: any) => {
					const startRaw =
					...
					const customerId = request.customerId?._id || request.customerId || request.customer_id;
					const packageId = request.packageId?._id || request.packageId || request.package_id;

					return {
						id: request._id || request.id,
						name: request.notes || request.name || 'Event',
						date: startRaw || '',
						status,
						capacity: request.deposit?.amount || 0,
						attendees: 0,
						customerId,
						packageId,
						startDate: startRaw || '',
				...
				setError(response.message || 'Error loading events');
			}
		} catch (err: any) {
	...
	const fetchOptions = async () => {
		try {
			...
			if (customersRes.status === 'success' && customersRes.data) {
				setCustomers(customersRes.data.customers || []);
			}
			if (packagesRes.status === 'success' && packagesRes.data) {
	...
	const handleSaveEvent = async () => {
		try {
			...
			if (editingId) {
				const response = await updateRequest(editingId, payload);
	...
	const handleDeleteEvent = async (eventId: string) => {
		...
		try {
			const reason = prompt('Motivo de cancelación', 'Cancelado por el administrador');
			if (reason === null) return;
			const response = await cancelRequest(eventId, reason);
			if (response.status !== 'success') {
				alert('Error: ' + response.message);
				return;
			}
				...
				<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
					<div className="bg-[#121212] border border-[#333] rounded-lg p-6 w-full max-w-md">
						...
						<div className="space-y-4">
								...
								>
									...
									{customers.map((customer) => (
										<option key={customer._id || customer.id} value={customer._id || customer.id}>
											{customer.name?.firstName || ''} {customer.name?.firstSurname || ''} ({customer.email || 'No email'})
										</option>
									))}
								</select>
							</div>
							...
							<div>
								<label className="block text-gray-400 text-sm font-medium mb-2">Package</label>
								<select
							...
							<div>
								<label className="block text-gray-400 text-sm font-medium mb-2">Deposit Amount</label>
								<input
									type="number"
							...
							<div>
								<label className="block text-gray-400 text-sm font-medium mb-2">Deposit Method</label>
								<select
